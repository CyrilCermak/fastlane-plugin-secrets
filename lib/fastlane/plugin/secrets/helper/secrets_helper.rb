    ...
    class SecretsHelper
      ...
      def self.generate_template(file)
        open(file, 'w') do |f|
          ...
          f << "    private let bytes: [[UInt8]] = [[/* SECRETS */]]\n"
          f << "\n"
          f << "    private init() {}\n"
          f << "\n"
          f << "    func string(forKey key: String) -> String? {\n"
          f << "        guard let index = bytes.index(where: { String(data: Data(bytes:$0), encoding: .utf8) == key }),\n"
          f << "              let value = decrypt(bytes[index + 1]) else { return nil }\n"
          f << "        return String(data: Data(bytes: value), encoding: .utf8)\n"
          f << "    }\n"
          f << "\n"
          f << "    private func decrypt(_ input: [UInt8]) -> [UInt8]? {\n"
